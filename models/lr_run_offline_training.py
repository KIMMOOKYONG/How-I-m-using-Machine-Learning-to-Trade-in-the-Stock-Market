...
logger = logging.getLogger("__LR_offline_training__")

class LR_training:

    def __init__(self, model_version, tickers = "KOSPI50", threshold = 0.98, start_date = None, end_date = None):
        ...
        dow = []        
        tickers = pd.read_csv(f"{self.curr_dir}/{tickers}.csv", encoding="euc-kr", dtype=str)
        tickers = list(tickers["종목코드"])
